...
library(readr) 
data <- read.csv("alldata_together_optimizedforanalysis_Kopie.csv", sep = ";") 
library(magrittr) 
library(dplyr)
```
...
```{r}
pdf("my_plot1.pdf")
hist(filtered_data$Age,main = "Age Distribution of Survey Participants",xlab = "Age", ylab="Number of participants")
dev.off
```
...
pdf("my_plot4.pdf")
boxplot(filtered_data$final_score~filtered_data$Gender,xlab = "Gender", ylab = "risk-taking index", main="Risk-Taking Index by Gender")
dev.off
```
